...
export default function EditVideoPage() {
  ...
  const loadVideo = async () => {
    try {
      ...
      const response = await fetch(`/api/doctor-videos/${videoId}`);
      if (response.ok) {
        const result = await response.json();
        const video = result.data || result;
        
        console.log('🎬 Video loaded for edit:', video);
        ...
        alert(`Video yüklenemedi: ${error.error}`);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.video_url.trim()) {
      newErrors.video_url = 'Video dosyası zorunludur';
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log('🎬 Updating doctor video:', submitData);
      ...
      if (response.ok) {
        ...
        alert(`Hata: ${result.error || 'Video güncellenemedi'}\n${result.details || ''}`);
      }
    } catch (error) {
      console.error('❌ Update error:', error);
      alert(`Update error: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      console.log('🎬 Deleting doctor video:', videoId);
      ...
      if (response.ok) {
        ...
        alert(`Silme hatası: ${result.error || 'Video silinemedi'}`);
      }
    } catch (error) {
      console.error('❌ Delete error:', error);
      alert(`Delete error: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}`);
    } finally {
      setDeleting(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-slate-600">Video yükleniyor...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Video Başarıyla Güncellendi!</h2>
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold text-slate-900">Video Düzenle</h1>
          <p className="text-slate-600 mt-1">Video bilgilerini güncelleyin</p>
        </div>
        <Button
